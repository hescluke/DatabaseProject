...
<script type="text/javascript" src="js/texiao.js"></script>
	...
	<script type="text/javascript">
		function delOne(song_id, user_id) {
			var sure = window.confirm("确定删除改选项吗？");
			if (sure) {
				window.location.href = "${pageContext.request.contextPath}/client/ClientServlet?op=delFavorite&song_id="
						+ song_id+"&user_id="+user_id;
				alert("删除成功！！");
			}
		}
	</script>
			...
			<c:forEach items="${favorite}" var="f" varStatus="vs">
				<tr class="active">
					...
					<td><img style="width: 80px; height: 100px"
						src="${pageContext.request.contextPath}/img/books/${f.song.song_name}.png" />
					</td>
					<td>${f.song.song_name}</td>
					<td>${f.song.category.category_name}</td>
					<td>${f.song.song_artist}</td>
					<td>${f.song.song_press}</td>
					<td><span style="color: rgb(198, 46, 45); font-weight: bold">￥${f.song.song_price}</span></td>
					<td><a href="javascript:delOne('${f.song.song_id}','${f.user.id}')">删除</a></td>
				</tr>
			</c:forEach>
		</table>
<%--	</c:if>--%>
